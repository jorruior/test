...
def split_chromosomes(chroms, trial=None, fixed_val=None):
    ...
    Otherwise: test={16,1,X}, val=fixed_val or first 3 remaining, train=rest.
    """
    ...
    if trial is not None:
        ...
        if len(parts) == 3:
            # Format: train_chr,val_chr,test_chr
...
def compute_shared_val_chromosomes(training_tracks, args, biotype):
    ...
    if _RANK == 0:
        ...
        if len(shared) >= 3:
            ...
            candidates = shared  # use whatever is available
...
class TranscriptDataset(Dataset):
    ...
    def __init__(self, sequences, index_array, inp_mmap, out_mmap,
                 seq_len, nbins, nosequence=False, track_id=0):
...
class RiboDataModule(LightningDataModule):
    ...
    def __init__(self, args, training_tracks, val_chroms, test_tracks=None):
        super().__init__()
        self.args            = args
        self.training_tracks = training_tracks
        self.test_tracks     = test_tracks or []  # tracks with dataset="test"
        self.val_chroms_norm = set(val_chroms)
        self._train_ds = None
        self._val_ds   = None
        self._test_ds  = None       # hold-out chroms from training tracks
        self._test_new_ds = None    # test-tagged tracks (all chroms)
        self._mmap_refs = []
        self.track_names = []       # track_id -> (species, tissue)
        self.test_new_track_names = []  # for test-tagged tracks
    ...
    def setup(self, stage=None):
        ...
        for track in self.training_tracks:
            ...
            _dbg(f"LOADED {sp}/{ti} (track_id={track_id}): total={len(df_all)} "
                 f"biotype_match={n_bt} train={n_tr} val={n_vl} test={n_te}")
        ...
        # ── Load test-tagged tracks (use ALL chromosomes) ─────────────
        test_new_parts = []
        for track in self.test_tracks:
            ...
            if not os.path.exists(inp_npy) or not os.path.exists(out_npy):
                _dbg(f"SKIP test track {sp}/{ti}: missing npy")
                continue

            df_all = load_coords(sp)
            if df_all is None or df_all.empty:
                _dbg(f"SKIP test track {sp}/{ti}: no coordinates")
                continue
            ...
            if inp_mm.shape[0] != len(df_all):
                _dbg(f"SKIP test track {sp}/{ti}: shape mismatch")
                continue

            bt_ok = _biotype_mask(df_all, self.args.biotype)
            sequences = df_all["sequence"].values

            # Use ALL chromosomes for test-tagged tracks
        ...
        if self._test_new_ds:
            ...
            print(f"  Test-tagged tracks: none\n")
    ...
    def test_new_dataloader(self):
        """Dataloader for test-tagged tracks (unseen samples, all chroms)."""
        if self._test_new_ds is None or len(self._test_new_ds) == 0:
            return None
        return self._make_loader(self._test_new_ds, shuffle=False, drop_last=True)
...
class RiboModel(LightningModule):
    ...
    def __init__(self, args):
        ...
        # Per-track test accumulators (populated at test time)
        self._test_per_track = {}   # track_id -> {loss_sum, loss_n, pcc_metric}
        self._track_names    = None  # set from DataModule before testing
        self._test_phase     = "holdout"  # "holdout" or "new_samples"
    ...
    def _get_test_track(self, tid):
        """Get or create per-track test accumulator."""
        tid = int(tid)
...
def main():
    ...
    dm = RiboDataModule(args, training_tracks, val_chroms, test_tracks=test_tracks)
    ...
    if not args.test:
        ...
        # Phase 1: Hold-out test chromosomes (training tracks aggregated)
        if dm._test_ds and len(dm._test_ds) > 0:
            if _RANK == 0:
                print("--- Phase 1: Hold-out chromosomes ---\n")
            model._test_phase = "holdout"
            model._track_names = dm.track_names
            trainer.test(model, dataloaders=dm.test_dataloader(),
                         ckpt_path=ckpt)
        else:
            if _RANK == 0:
                print("  No hold-out test data (skipping phase 1)\n")

        # Phase 2: Test-tagged tracks (per sample, all chromosomes)
        if dm._test_new_ds and len(dm._test_new_ds) > 0:
            if _RANK == 0:
                print("\n--- Phase 2: Test-tagged samples (all chroms) ---\n")
            model._test_phase = "new_samples"
            model._track_names = dm.test_new_track_names
            trainer.test(model, dataloaders=dm.test_new_dataloader(),
                         ckpt_path=ckpt)
        else:
            if _RANK == 0:
                print("  No test-tagged tracks in tracks file (skipping phase 2)\n")
