...
Key design choices
------------------
1.  Lightning owns the process group (no manual dist.init_process_group).
    SLURMEnvironment is passed explicitly so Lightning does NOT re-spawn
    processes via its subprocess launcher — srun already did that.

2.  Dataset pre-filters all-N transcripts at __init__, so __getitem__
    never returns None.  No custom collate_fn needed.

3.  One-hot encoding uses a uint8 lookup table (no Python loop).

4.  Mmap arrays are sliced directly; .astype(float32) is a no-op when
    the mmap is already float32.

5.  save_hyperparameters() gets an explicit dict, not the full Namespace.

6.  enable_checkpointing=True on ALL ranks.  ModelCheckpoint callback
    on ALL ranks.  Lightning gates actual I/O to rank-0 internally.

7.  ALL self.log() calls use sync_dist=False.  This is critical: using
    sync_dist=True triggers an allreduce, and if any rank has a
    different batch count the allreduce hangs forever (NCCL timeout).
    Each rank logs its own local value; callbacks see it on every rank.

8.  Validation uses drop_last=True to guarantee all ranks process the
    exact same number of batches.

9.  Pearson metric uses float64 accumulators.  Its .compute() does NOT
    trigger distributed reduction (we disabled sync_dist on the log).

10. Loss = weighted_MSE + 0.2 * (1 - per_sample_PCC).

Author: Jorge Ruiz-Orera (rewritten)
"""

# ═══════════════════════════════════════════════════════════════════════════
# § 0  Early environment setup
# ═══════════════════════════════════════════════════════════════════════════
...
# Keep a real print for debug on all ranks (writes to stderr)
def _dbg(msg):
...
def _pt_to_npy(pt_path: str) -> Optional[str]:
...
def convert_all_tracks(training_tracks, args):
    ...
    # All ranks poll until files appear
...
def load_coords(species, biotype="protein_coding"):
    ...
    with open(path) as f:
        for line in f:
            ...
            if len(p) < 8:
                continue
            if bt_filter is not None and p[5] not in bt_filter:
                continue
            rows.append({
                "chr": p[0], "start": int(p[1]), "end": int(p[2]),
                "id": p[3], "sequence": p[7], "species": species,
            })
...
def compute_shared_val_chromosomes(training_tracks, args, biotype):
    ...
    if _RANK == 0:
        per_species = {}
        for t in training_tracks:
            df = load_coords(t["species"], biotype)
            if df is not None:
...
class RiboDataModule(LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        _dbg(f"setup() cwd={os.getcwd()} tracks_dir={self.args.tracks_dir} "
             f"n_tracks={len(self.training_tracks)}")
        ...
        for track in self.training_tracks:
            ...
            if not os.path.exists(inp_npy) or not os.path.exists(out_npy):
                _dbg(f"SKIP {sp}/{ti}: missing npy "
                     f"(inp={os.path.exists(inp_npy)} out={os.path.exists(out_npy)})")
                continue

            df = load_coords(sp, self.args.biotype)
            if df is None or df.empty:
                _dbg(f"SKIP {sp}/{ti}: no matching coordinates")
                continue
            ...
            if inp_mm.shape[0] != len(df):
                _dbg(f"SKIP {sp}/{ti}: shape mismatch "
                     f"mmap={inp_mm.shape[0]} coords={len(df)}")
                continue

            sequences   = df["sequence"].values
            norm_chroms = np.array([_norm_chr(c) for c in df["chr"].values])

            train_chr, val_chr, _ = split_chromosomes(
                df["chr"].unique().tolist(),
            ...
            tr_idx = np.where(np.isin(norm_chroms, list(tr_set)))[0]
            vl_idx = np.where(np.isin(norm_chroms, list(vl_set)))[0]
            ...
            _dbg(f"LOADED {sp}/{ti}: {n_tr} train, {n_vl} val")
    ...
    def val_dataloader(self):
        if self._val_ds is None or len(self._val_ds) == 0:
            return None
        # drop_last=True ensures all ranks process the same number of
        # batches, preventing NCCL allreduce hangs
...
class RiboModel(LightningModule):
    ...
    # ------------------------------------------------------------------
    # CRITICAL: All self.log() calls use sync_dist=False.
    # sync_dist=True triggers an NCCL allreduce.  If any rank has a
    # different number of batches, the allreduce hangs forever.
    # ------------------------------------------------------------------
    ...
    def on_train_epoch_end(self):
        ...
        # sync_dist=False — no allreduce, no hang risk
...
def main():
    ...
    # ── DDP Strategy ──────────────────────────────────────────────────
    # SLURMEnvironment tells Lightning that srun already launched one
    # process per GPU — do NOT use the subprocess launcher.
