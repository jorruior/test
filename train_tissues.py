...
def _pt_to_npy(pt_path):
    ...
    try:
        ...
        try: os.remove(npy_path + f".tmp.{os.getpid()}")
        except OSError: pass
        return None
    finally:
...
def main():
    ...
    save_dir = (
        ...
        f"_nt{len(tissue_vocab)}_seed{args.seed}{suffix}"
    )
    ...
    if not args.test:
        ...
        if dm._test_ds and len(dm._test_ds) > 0:
            if _RANK == 0: print("--- Phase 1: Hold-out chromosomes ---\n")
            model._test_phase = "holdout"
            model._track_names = dm.track_names
            trainer.test(model, dataloaders=dm.test_dataloader(),
                         ckpt_path=ckpt)
        else:
            if _RANK == 0: print("  No hold-out test data (skipping phase 1)\n")
        ...
        if dm._test_new_ds and len(dm._test_new_ds) > 0:
            ...
            if _RANK == 0: print("  No test-tagged tracks (skipping phase 2)\n")
